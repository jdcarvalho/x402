"""Unit tests for the server-side `BatchSettlementChannelManager`."""
...
) -> tuple[BatchSettlementChannelManager, BatchSettlementEvmScheme, _FakeFacilitator]:
...
class TestClaim:
    ...
    def test_batches_into_max_claims_per_batch(self):
        ...
        cm, _, fac = _make_manager(seed_channels=channels)
    ...
    def test_custom_selector_filters_targets(self):
        ...
        cm, _, fac = _make_manager(seed_channels=[ch1, ch2])
...
class TestClaimAndSettle:
    ...
    def test_runs_settle_when_claims_present(self):
        ...
        cm, _, fac = _make_manager(seed_channels=[ch], facilitator=fac)
        out = cm.claim_and_settle()
